...
def process_one_no_timeout( fold, cnt, tcnt, binary_hashes, cmakefiles_bins, tots, lck  ):
    ...
    #if 'z80asm-' not in fold: return

    print("fid: ", cnt, tcnt, flush=True )
    fpath = folder_oss_compiled+fold+'/' + file_extension
    ...
    gcands      = 0
    bad_fold    = False
    #files = list_files( folder_oss_compiled+fold, 
    #        good_file_pred, 
    #        lambda x: x)
    files = [ f for f in glob.glob(folder_oss_compiled+fold+'/OUT/**', recursive=True) if good_file_pred(f) ]
    ...
    for f in files:
        ...
        print('Potential:', f, flush=True)

        #if 'simple_compress' not in f: continue             # REMVOE LATER


        # get testcases
        if not os.path.exists(folder_temp_testcases + '/' + fold):
            os.mkdir(folder_temp_testcases + '/' + fold)
        tc_candidates = []
        base = os.path.basename(f)
        zip_files = glob.glob(folder_oss_compiled+fold+'/OUT/'+base+'**.zip')
        if len(zip_files) == 0:
            zip_files = glob.glob(folder_oss_compiled+fold+'/OUT/*.zip')
        if len(zip_files) > 0:
            #os.system('unzip -o ' + zip_files[0] + ' -d ' + folder_temp_testcases + '/' + fold + '/' + base )
            # rewrite os.system call using subprocess and do not output
            p = subprocess.Popen( 'unzip -o ' + zip_files[0] + ' -d ' + folder_temp_testcases + '/' + fold + '/' + base , shell=True, stdout= subprocess.DEVNULL, stderr= subprocess.DEVNULL)
            p.wait()
            tc_candidates = [ f for f in glob.glob(folder_temp_testcases + '/' + fold + '/' + base + '/**', recursive=True) if os.path.isfile(f) ]
        else:
            for root, dirs, files in os.walk(folder_oss_compiled+fold):
                for file in files:
                    ff = os.path.join(root, file)
                    if os.path.isfile(ff):
                        tc_size = os.path.getsize(ff)
                        if tc_size == 0 or tc_size > max_tc_size : 
                            continue
                        tc_candidates.append( f )
            if len(tc_candidates) == 0:
                os.mkdir(folder_temp_testcases + '/' + fold + '/' + base )
                fname = folder_temp_testcases + '/' + fold + '/' + base + '/simple_tc.txt'
                with open(fname, 'w') as fff: fff.write('01234567')
                tc_candidates = [ fname ]
        ...
        probs = [ 1.0/len(tc_candidates) ] * len(tc_candidates)
        ...
        if len(dyn_libs_missing) == 1:
            ...
            if dyn_libs_missing[0] not in missing_libs:
                if tot_path is None:
                    search_string = 'find ' + folder_oss_compiled+fold + ' -name '+dyn_libs_missing[0]
                else:
                    search_string = 'find ' + folder_oss_compiled+fold + ' -path *'+tot_path
                try:
                    p = subprocess.Popen( 'find ' + folder_oss_compiled+fold + ' -name '+dyn_libs_missing[0] , shell=True, executable='/bin/bash', stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
                    out, err = p.communicate(input=b'one\ntwo\nthree\nfour\nfive\nsix\n',timeout=2)
                    out = out.decode('utf-8', errors='ignore') 
                    print('out:',out)
                except:
        ...
        print('TESTCASES for ', base, len(tc_candidates))
        if len(tc_candidates) > config_no_tc:
            ch = numpy.random.choice(tc_candidates, config_no_tc, p=probs )
        else:
            ch = tc_candidates
        tcno = 0
...
if __name__ == '__main__':
    ...
    if not os.path.exists(folder_oss_compiled):
        print('cannot find compiled folder', folder_oss_compiled)
        exit()
    ...
    seconds_per_binary_param = 600       # needs to be integer !
    ...
    folder_temp_testcases = cur_folder_abs + '/temp_testcases'
    if os.path.exists(folder_temp_testcases):
        shutil.rmtree(folder_temp_testcases)
    os.mkdir(folder_temp_testcases)
    ...
    alldirs = []
    for f in glob.glob(folder_oss_compiled + '/*'):
        if os.path.isdir(f):
            if '_defaultc' not in f: continue                # check only compiled with AFL compiler       
